...
export function AttachmentsListPreview({setAttachments}: {setAttachments: (att: FileList | null) => void}) {
  ...
  return (
    <div className="flex gap-2 mt-8 pb-6 overflow-x-scroll scrollbar-thumb-red-300 hover:scrollbar-thumb-red-200 active:scrollbar-thumb-red-400 scrollbar-track-transparent scrollbar-thin">
      <div className="w-fit">
        <label className="flex flex-col gap-4" htmlFor="attachments">
          ...
          <div className="flex justify-center items-center w-52 h-28 border-dashed  border border-rose-400 rounded-md hover:bg-slate-200 active:bg-slate-300 transition-colors cursor-pointer">
      ...
      <div className={`flex items-end gap-2 ${attachmentsList && "-ml-[60px]"}`}>
        {
          attachmentsList?.map(attachment => (
            <div 
              key={attachment.name}
              className="flex justify-center items-center w-52 h-28 bg-slate-300 rounded-md overflow-hidden"
            >
              <img 
